...
describe.skipIf(!hostedProject)(
  "the assistant's record and audit tools on the hosted project",
  () => {
    ...
    describe("records on the hero residents", () => {
      it("lists lab results with the latest of each test, and narrows by test name", async () => {
        ...
        for (const entry of result!.latestByTest) {
          ...
          if (entry.previous)
            expect(entry.previous.resultedOn <= entry.latest.resultedOn).toBe(true);
        }
